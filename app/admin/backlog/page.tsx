                ...
                sortedItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <button
                        onClick={async () => {
                          if (!item.done && !item.in_progress) {
                            setItems((prev) =>
                              prev.map((i) =>
                                i.id === item.id
                                  ? { ...i, in_progress: true, done: false }
                                  : { ...i, in_progress: false }
                              )
                            );
                            await saveItem(item.id, "in_progress", true);
                          } else if (item.in_progress) {
                            setItems((prev) =>
                              prev.map((i) =>
                                i.id === item.id
                                  ? { ...i, in_progress: false, done: true }
                                  : i
                              )
                            );
                            setSaving((prev) => ({ ...prev, [item.id]: true }));
                            await updateBacklogItem(item.id, { in_progress: false, done: true });
                            setSaving((prev) => ({ ...prev, [item.id]: false }));
                          } else {
                            setItems((prev) =>
                              prev.map((i) =>
                                i.id === item.id
                                  ? { ...i, done: false, in_progress: false }
                                  : i
                              )
                            );
                            await saveItem(item.id, "done", false);
                          }
                        }}
